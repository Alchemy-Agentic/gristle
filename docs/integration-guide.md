...
| `USES_TOKEN` | Function | Token | A component/function uses a design token — resolved from a literal `className` utility class (a Tailwind prefix→namespace map: `bg-primary`→`--color-primary`, `p-4`→`--spacing-4`, `text-lg` vs `text-primary` disambiguated by which token exists) or a `var(--x)` reference in its body. **Name-gated**: only a class/var that resolves to a real `Token` yields an edge (stock utilities like `flex`, default-scale classes, and arbitrary `bg-[#fff]` values produce nothing). Feeds each token's `used_by` count and the dead-token / retire-candidate signal |
| `GATES` | Flag | Function | A feature flag gates a function that checks it, via a configured check call (`useFeatureFlag('K')`, `isFeatureFlagEnabled(sb, 'K', id)`), a direct flag-table read (`.from('feature_flags').eq('id','K')`), or a registry/cache member read (`featureFlags.K` / `runtimeFlagCache.K`). A key passed as a `const` resolves to the const's value. The count mirrors `Flag.gates_count`; a registry flag with zero gates is a retire candidate. Key extraction is position-configured, so a non-key string argument (a log tag) is never taken as the key |
...
- **Next.js:** App router file conventions — `app/api/users/route.ts` → `GET /api/users` (linked to its `GET`/`POST`/… export), and `app/users/[id]/page.tsx` → `GET /users/:id` linked via `HANDLES` to the file's **default-export page component**, so `route → page component → CALLS`/`RENDERS` → `USES_MODEL` traces a Server Component page's data access. Layout/loading/error files link the same way; an anonymous or HOC-wrapped default export (no name to resolve) stays unlinked
