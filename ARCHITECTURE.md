...
src/gristle/
  ...
  graph/
    ...
    schema.py              # Index creation (45 property indexes, 2 full-text)
  parsers/
...
- **Serverless entry files** (`_serverless_entry`): a file whose *location* makes it an HTTP endpoint, regardless of how the handler registers internally. Two conventions: **Supabase edge functions** (`supabase/functions/<name>/index.ts`, non-`_`-prefixed) → `POST /<name>`; **Cloudflare Workers / services** (`workers/<name>/src/index.ts`, `services/<name>/src/index.ts`, `apps/<name>/worker/index.ts`) → `ALL /<name>` (a `fetch` handler serves all methods). The request handler is located from the top-level registration — `serve(handler)`/`Deno.serve(handler)`, any `serve`-prefixed wrapper (`serveWithInstrumentation('name', handler, {opts})`), `export default { fetch: handler }` / `{ async fetch(){} }`, `export default wrap(opts, { fetch })`, a `const handler = {...}` referenced by the default export, or `addEventListener('fetch', handler)` — taking the last *function-typed* argument (the handler is not always last). An inline handler is synthesized into a Function (so its calls chain to `USES_MODEL`/`CALLS_RPC`); a named handler is linked by name. Handlers that delegate to an internal Hono/Express app (`export default app`, `Deno.serve(app.fetch)`) keep the app's own routes instead of the envelope.
- **Entry points:** Next.js pages, Storybook stories, React components, serverless handlers (`handler` export), barrel-only hooks (`use*` in `index.ts`), `main()` exports, route handlers, functions called by a serverless handler — each sets `entry_point_reason`
...
5. **SQL migrations** (`.sql` files, tree-sitter-sql): `parse_sql_schema` extracts each `CREATE FUNCTION`'s body table access — `INSERT`/`UPDATE`/`DELETE` targets are `write`, `SELECT`/`FROM`/`JOIN` sources are `read` (write beats read; names schema-stripped). `_link_db_functions_to_models` then creates `DBFunction-[:USES_MODEL {access}]->Model`, name-gated on both ends (the SQL function must match a declared `DBFunction`, the table a known `Model`). This is where a stored procedure's real table mutations live — invisible to the generated types (signature only) — so a table written only by an RPC becomes a visible write target and `route → CALLS_RPC → DBFunction → USES_MODEL → Model` resolves end to end. tree-sitter-sql's plpgsql coverage is incomplete, so table access is collected from a **masked, standalone re-parse of each function's dollar-quoted body**: the body is extracted by text, its comments and nested string/dollar-quoted literals are blanked, then parsed alone. This recovers the ~23% of functions (e.g. `SECURITY DEFINER SET search_path` + `IF … RAISE` guards) that otherwise produce no AST node, while masking keeps it precise — a table named inside a `RAISE`/`EXECUTE` message string, or a `CREATE FUNCTION` inside a comment or dynamic-SQL string, is never read as live SQL. Signature params, `DECLARE`/loop vars, and CTE names are subtracted (a standalone body has no declaration context).
